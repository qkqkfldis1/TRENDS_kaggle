...
class TReNDsDataset(Dataset):
    ...
    def __init__(self, mode='train', fold_index = 0, feat_index=0):
        ...
        train = pd.read_csv('{}/train_scores_kd_73.csv'.format(root)).sort_values(by='Id')
        train_orig = pd.read_csv('{}/train_scores.csv'.format(root)).sort_values(by='Id')
        ...
        for col in cols:
            train_orig[col] -= target_stats[col][0]
            train_orig[col] /= target_stats[col][1]
            
            train[col] -= target_stats[col][0]
            train[col] /= target_stats[col][1]
        ...
        if self.mode=='train' or self.mode=='valid' or self.mode=='valid_tta':
            ...
            train_orig = train_orig[['Id', features[self.feat_index]]]
            ...
            data_orig = pd.merge(loadings, train_orig, on='Id').dropna()
            lbl_train_orig = np.asarray(data_orig[[features[self.feat_index]]])
            ...
            for i in tqdm(range(len(id_train))):
                ...
                fnc = fnc_train[i]
                deg = deg_train[i]
                lbl_orig = lbl_train_orig[i]
                lbl_kd = lbl_train[i]
                filename = os.path.join('{}/fMRI_train_npy/{}.npy'.format(root, id))
                self.all_samples.append([filename, fea, fnc, deg, lbl_orig, lbl_kd, str(id)])
                
            #self.orig_lbls = []
            #for i in tqdm(range(len(id_train))):
            #    lbl = lbl_train_orig[i]
            #    self.orig_lbls.append(lbl)
            ...
            labels_df = pd.read_csv("{}/train_scores_kd.csv".format(root))
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train" :
            filename, feat, fnc, deg, lbl_orig, lbl_kd, id =  self.all_samples[self.train_index[idx]]
            train_img = np.load(filename).astype(np.float32)
            train_img = train_img.transpose((3,2,1,0))
            # (53, 52, 63, 53)
            train_lbl_orig = lbl_orig
            train_lbl_kd = lbl_kd
            ...
            return torch.FloatTensor(train_img), \
                   ...
                   torch.FloatTensor(train_lbl_orig), \
                   torch.FloatTensor(train_lbl_kd)

        elif self.mode == "valid":
            filename, feat, fnc, deg, lbl_orig, lbl_kd, id =  self.all_samples[self.valid_index[idx]]
            #lbl = self.orig_lbls[self.valid_index[idx]]
