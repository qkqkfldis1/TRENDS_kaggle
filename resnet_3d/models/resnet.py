...
class ResNet3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.last_fc_1 = nn.Sequential(nn.Linear(self.fea_dim * 4, self.fea_dim * 4, bias=True))
        self.bn_fc_1 = nn.BatchNorm1d(self.fea_dim * 4)
        self.relu_fc_1 = nn.ReLU(inplace=True)
        
        self.last_fc_2 = nn.Sequential(nn.Linear(self.fea_dim * 4, self.fea_dim * 4, bias=True))
        self.bn_fc_2 = nn.BatchNorm1d(self.fea_dim * 4)
        self.relu_fc_2 = nn.ReLU(inplace=True)
        
        self.last_fc_3 = nn.Sequential(nn.Linear(self.fea_dim * 4, self.fea_dim * 4, bias=True))
        self.bn_fc_3 = nn.BatchNorm1d(self.fea_dim * 4)
        self.relu_fc_3 = nn.ReLU(inplace=True)
        
        self.last_fc = nn.Linear(self.fea_dim * 4, num_class, bias=True) #nn.Sequential(nn.Linear(self.fea_dim * 3, num_class, bias=True))
    ...
    def forward(self, x, x_feat, x_fnc, x_deg):
        ...
        x = torch.cat([x, x_feat, x_fnc, x_deg], dim=1)
       # x = self.relu_concat(x)
        
        res = self.last_fc_1(x)
        res = self.bn_fc_1(res)
        x = res + x
        x = self.relu_fc_1(x)
        
        res = self.last_fc_2(x)
        res = self.bn_fc_2(res)
        x = res + x
        x = self.relu_fc_2(x)   
        
        res = self.last_fc_3(x)
        res = self.bn_fc_3(res)
        x = res + x
        x = self.relu_fc_3(x) 
        
        out = torch.mean(
            torch.stack(
                [self.last_fc(self.high_dropout(x)) for _ in range(5)],
                dim=0,
            ),
            dim=0,
        )
